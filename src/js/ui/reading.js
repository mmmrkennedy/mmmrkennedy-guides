...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const solverState = new Map();
    const solverTimers = new Map();
    const solversVisible = new Set();
    ...
    function currentAnchorId() {
        ...
        ticksSinceMeasure++;
        if (scrollTop === lastScrollTop && ticksSinceMeasure < 10)
            return currentId;
        ...
        for (const el of anchors) {
            if (el.getBoundingClientRect().top > mid)
                break; // document order: the rest are lower
            if (el.id)
                found = el.id;
        }
     ...
     */
    function startSolverTracking() {
        const roots = Array.from(document.querySelectorAll("[data-solver-root], [id$='-react']"));
        if (roots.length === 0)
            return;
        const observer = new IntersectionObserver((entries) => {
            for (const entry of entries) {
                ...
                if (entry.isIntersecting)
                    solversVisible.add(el);
                else
                    solversVisible.delete(el);
            }
        }, { root: scroller, threshold: 0 });
        ...
        const timer = window.setInterval(() => {
            ...
            if (pending.size === 0 || tries >= SOLVER_POLL_TRIES)
                window.clearInterval(timer);
        }, SOLVER_POLL_MS);
    }
    function onSolverInput(root) {
        const stat = solverStats.get(root);
        if (stat)
            stat.e = 1;
        ...
        solverTimers.set(root, window.setTimeout(() => readSolverState(root), SOLVER_DEBOUNCE_MS));
    }
    function readSolverState(root) {
        const report = window.SolverReport?.read(root) ?? null;
        const stat = solverStats.get(root);
        if (!report || !stat)
            return;
        ...
        if (serialized === solverState.get(root))
            return;
    ...
    function startEventTracking() {
        document.addEventListener("click", (event) => {
            ...
            if (dest.host === window.location.host)
                record("nav", dest.pathname);
            else
                record("out", dest.host);
        }, true);
    }
    ...
    function shortMediaName(href) {
        const clean = href.split(/[?#]/)[0];
        return clean.split("/").slice(-2).join("/");
    }
    // ---- sending -------------------------------------------------------------
    function flush(force) {
        if (engaged === 0 && dwell.size === 0)
            return;
        if (!force && Date.now() - lastSendAt < MIN_SEND_GAP_MS)
            return;
        if (engaged === lastSentEngaged && lastSendAt !== 0)
    ...
    function payload() {
        ...
        for (const stat of solverStats.values()) {
            if (!stat.name)
                continue; // never mounted; nothing happened worth a row
