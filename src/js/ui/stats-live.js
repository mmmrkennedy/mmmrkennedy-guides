...
document.addEventListener("DOMContentLoaded", () => {
    if (!document.body.hasAttribute("data-stats-live"))
        return;
    const section = document.getElementById("live-views");
    const status = section?.querySelector(".stats-live__status");
    const chart = section?.querySelector(".stats-live__chart");
    if (!section || !status || !chart)
        return;
    const TOP_N = 10;
    // Published guides, keyed by normalized path, from the table this page already renders.
    const names = new Map();
    document.querySelectorAll(".stats-table a[href]").forEach((a) => {
    ...
    fetch("/api/trending")
        ...
        for (const item of data.ranked || []) {
            ...
            if (!hit)
                continue; // a solver, or a page no longer published
