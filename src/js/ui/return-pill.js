...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const io = "IntersectionObserver" in window
        ? new IntersectionObserver((entries) => {
            for (const en of entries) {
                if (en.target !== originSection)
                    continue;
                if (!en.isIntersecting)
                    leftOrigin = true;
                else if (leftOrigin && armed)
                    hide();
            }
        }, { root: contentWindow })
        : null;
    ...
    function show(fromLabel, section) {
        ...
        if (io && originSection)
            io.unobserve(originSection);
        originSection = section;
        leftOrigin = false;
        if (io && originSection)
            io.observe(originSection);
        armed = true;
    ...
    function sectionFor(anchor) {
        return anchor.closest(".content-container");
    }
    function labelFor(section) {
        const title = section?.querySelector(".title-tier-1, h2, h3, h4");
    ...
    document.addEventListener("scrollmanager:navigate", (e) => {
        ...
        if (!anchor || !anchor.classList.contains("link-to-page"))
            return;
