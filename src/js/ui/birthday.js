...
document.addEventListener("DOMContentLoaded", () => {
    const banner = document.querySelector(".site-birthday[data-birthday]");
    if (!banner)
        return;
    const parts = /^(\d{4})-(\d{2})-(\d{2})$/.exec(banner.dataset.birthday ?? "");
    if (!parts)
        return; // no release data in the build, so stay hidden
    ...
    if (age < 1)
        return; // the launch day itself isn't a birthday
    const slot = banner.querySelector(".site-birthday__age");
    if (slot)
        slot.textContent = ordinal(age);
    banner.hidden = false;
    /** 1 -> "1st", 3 -> "3rd", 11 -> "11th". */
    function ordinal(n) {
