 ...
 *
 * Block comments, not `--`, throughout this file. Migrations here are applied by
 * pasting into the D1 Console, and a paste that loses its newlines turns every
 * `--` into a comment that swallows the rest of the statement, closing paren and
 * all ("incomplete input: SQLITE_ERROR"). This form survives that.
 *
 ...
 * `.content-container[id]` blocks, plus the `p.title-tier-2[id]` sub-headings
 * inside them (the four bows, the six quest steps), and those ids are already
 * stable and unique, so they make free analytics keys.
 *
...
CREATE TABLE IF NOT EXISTS pageviews (
    ...
    prev      INTEGER NOT NULL DEFAULT 0, /* 1 = preview deploy or local dev, not real traffic */
